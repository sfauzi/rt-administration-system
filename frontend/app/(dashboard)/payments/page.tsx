...
export default function PaymentsPage() {
  ...
  useEffect(() => {
    if (!deletePayment.isPending && deletePayment.isSuccess) {
      refetch();
    }
  }, [deletePayment.isPending, deletePayment.isSuccess, refetch]);
            ...
            >
              ...
              {status !== '' && (
                <span className={`ml-1.5 text-xs px-1.5 py-0.5 rounded-full ${
                  ...
                  {payments.filter((p: Payment) => p.status === status).length}
                </span>
              )}
            </button>
          ))}
